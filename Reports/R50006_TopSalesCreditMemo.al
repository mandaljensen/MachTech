report 50007 TopStandardSalesCreditMemo
{
    // RDLCLayout = './StandardSalesInvoice.rdlc';
    WordLayout = './Reports/SalgskreditnotaMachTech.docx';
    ...
    dataset
    {
        dataitem(Header; "Sales Cr.Memo Header")
        {
            ...
            column(CompanyVATRegNo_Lbl; VATRegNoLbl)
            {
            }
            column(CompanyVATRegistrationNo; CompanyInfo.GetVATRegistrationNumber)
            {
            }
            column(CompanyVATRegistrationNo_Lbl; VATRegNoLbl)
            {
            }
            ...
            column(BillToCustVatNo_Lbl; Cust.fieldcaption("VAT Registration No."))
            {
                // rsbdk Added
            }
            column(BillToCustVatNo; Cust."VAT Registration No.")
            {
                // rsbdk Added
            }
            column(JobNoHeader; JobNo)
            {
                // rsbdk Added
            }
            ...
            dataitem(Line; "Sales Cr.Memo Line")
            {
                ...
                begin
                    ...
                    if ("Job No." <> '') then //and (not EnvironmentInfo.IsSaaS) then
                        JobNo := ''
                    else
                        JobNo := "Job No.";
                    if ("Job Task No." <> '') then //and (not EnvironmentInfo.IsSaaS) then
                        JobTaskNo := ''
                    else
    ...
    var
        ...
        InvDiscBaseAmtLbl: Label 'Credit Memo Discount Base Amount';
        InvDiscountAmtLbl: Label 'Credit Memo Discount';
        ...
        YourSalesInvoiceLbl: Label 'Your Credit Memo';
        ...
        AlreadyPaidLbl: Label 'The Credit Memo has been paid.';
        PartiallyPaidLbl: Label 'The Credit Memo has been partially paid. The remaining amount is %1', Comment = '%1=an amount';
        FromLbl: Label 'From';
    ...
    local procedure FormatAddressFields(var SalesCreditMemoHeader: Record "Sales Cr.Memo Header")
    begin
        FormatAddr.GetCompanyAddr(SalesCreditMemoHeader."Responsibility Center", RespCenter, CompanyInfo, CompanyAddr);
        FormatAddr.SalesCrMemoBillTo(CustAddr, SalesCreditMemoHeader);
        ShowShippingAddr := FormatAddr.SalesCrMemoShipTo(ShipToAddr, CustAddr, SalesCreditMemoHeader);
    end;
    ...
    local procedure OnAfterLineOnPreDataItem(var SalesCreditMemoHeader: Record "Sales Cr.Memo Header"; var SalesCreditMemoLine: Record "Sales Cr.Memo Line")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeLineOnAfterGetRecord(var SalesCreditMemoHeader: Record "Sales Cr.Memo Header"; var SalescreditMemoLine: Record "Sales Cr.Memo Line")
    begin
    end;
    ...
    local procedure OnAfterGetSalesHeader(SalesCrMemoHeader: Record "Sales Cr.Memo Header")
    begin
    end;
