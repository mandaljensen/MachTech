report 50006 TopStandardSalesInvoice
{
    // RDLCLayout = './StandardSalesInvoice.rdlc';
    WordLayout = './Reports/SalgsfakturaMachTech.docx';
    ...
    var
        SalesInvoiceLineLocal: Record "Sales Invoice Line";
        JobNoValue: Code[20];
    begin
        SalesInvoiceLineLocal.Reset();
        SalesInvoiceLineLocal.SetRange("Document No.", Header."No.");
        if SalesInvoiceLineLocal.FindSet(false, false) then
            repeat
                if SalesInvoiceLineLocal."Job No." <> '' then
                    JobNoValue := SalesInvoiceLineLocal."Job No.";
            until ((SalesInvoiceLineLocal.Next() <= 0) or (JobNoValue <> ''));
